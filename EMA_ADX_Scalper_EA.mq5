...
//+------------------------------------------------------------------+
#property copyright "EMA_ADX_Scalper_EA v1.0"
...
input int    InpSL_Points    = 500;    // Stop Loss en points
input int    InpTP_Points    = 1000;   // Take Profit en points
...
void OnTick()
  {
   ...
   double sl_pts = InpSL_Points * point;
   double tp_pts = InpTP_Points * point;
   ...
   if(crossBuy)
     {
      double sl = NormalizeDouble(ask - sl_pts, _Digits);
      double tp = NormalizeDouble(ask + tp_pts, _Digits);
      if(trade.Buy(InpLotSize, _Symbol, ask, sl, tp,
                   StringFormat("EMA ADX BUY | ADX:%.1f -DI:%.1f", adxCur, dimCur)))
        {
         g_adxState = 0;  // reset cycle
         g_lastTime = barTime;
         Print(StringFormat("[BUY]  Prix:%.2f | SL:%.2f | TP:%.2f | ADX:%.1f | -DI:%.1f",
               ask, sl, tp, adxCur, dimCur));
        }
      else
     ...
     {
      double sl = NormalizeDouble(bid + sl_pts, _Digits);
      double tp = NormalizeDouble(bid - tp_pts, _Digits);
      if(trade.Sell(InpLotSize, _Symbol, bid, sl, tp,
                    StringFormat("EMA ADX SELL | ADX:%.1f +DI:%.1f", adxCur, dipCur)))
        {
         g_adxState = 0;  // reset cycle
         g_lastTime = barTime;
         Print(StringFormat("[SELL] Prix:%.2f | SL:%.2f | TP:%.2f | ADX:%.1f | +DI:%.1f",
               bid, sl, tp, adxCur, dipCur));
        }
      else
