...
//|                          Expert Advisor — Version 1.6            |
//|                                                                  |
//|  DETECTION DE TREND — 2 options (InpUseSupertrend) :            |
//|                                                                  |
//|  Option 1 : false → EMAs (comportement v1.5)                    |
//|    InpUseEMA200=true  → EMA20 > EMA46 > EMA200                  |
//|    InpUseEMA200=false → EMA20 > EMA46 seulement                 |
//|                                                                  |
//|  Option 2 : true → Supertrend                                   |
//|    BUY  : Supertrend trend = +1 (ligne verte sous le prix)      |
//|    SELL : Supertrend trend = -1 (ligne rouge au-dessus)         |
//|    Parametres : InpST_Period, InpST_Mult                        |
//|                                                                  |
//|  Les EMAs restent affichees meme en mode Supertrend             |
//|  (elles ne sont juste plus utilisees pour le filtre trend)      |
//|                                                                  |
//|  RAPPEL COUCHES (inchangees depuis v1.5) :                      |
//|  COUCHE 1 : RSI filtre entree                                   |
//|  COUCHE 2 : ATR SL/TP dynamique                                 |
//|  COUCHE 3a: Breakeven automatique                               |
//|  COUCHE 3b: Trailing Stop ATR                                   |
//+------------------------------------------------------------------+
#property copyright "EMA_ADX_Scalper_EA v1.6"
...
input double InpRSI_BuyMin   = 50.0; // RSI minimum pour BUY
...
input double InpRSI_SellMax  = 50.0; // RSI maximum pour SELL
...
input double InpTP_ATR       = 3.0;  // TP = ATR x multiplicateur
...
input double InpTrail_ATR    = 1.0;  // Distance trail = ATR x multiplicateur
...
int h_ST_ATR = INVALID_HANDLE;  // ATR dedie au Supertrend (si periode differente)

// Machine a etats ADX
int      g_adxState = 0;
datetime g_lastTime = 0;

// Etat interne Supertrend (ratchet persistant)
double   g_st_up    = 0;
double   g_st_dn    = 0;
int      g_st_trend = 1;   // +1 = bull, -1 = bear

//+------------------------------------------------------------------+
int OnInit()
  {
   // EMAs — toujours creees (utilisees ou non selon le mode)
   ...
   // ATR Supertrend — reutilise h_ATR si meme periode, sinon cree un handle dedie
   if(InpUseSupertrend)
     {
      if(InpST_Period == InpATR_Period && h_ATR != INVALID_HANDLE)
        {
         h_ST_ATR = h_ATR;  // reutilisation
        }
      else
        {
   ...
   Print("=====================================================");
   Print("EMA ADX Scalper EA v1.6 | ", _Symbol, " | ", EnumToString(Period()));
...
void OnDeinit(const int reason)
  {
   ...
   // Liberer h_ST_ATR seulement si c est un handle independant
...
void OnTick()
  {
   ...
   //================================================================
   // DETECTION DE TREND
   // InpUseSupertrend = false → EMAs (Option 1)
   // InpUseSupertrend = true  → Supertrend (Option 2)
   //================================================================
   bool emaBull = false;
   bool emaBear = false;
   ...
   if(!InpUseSupertrend)
   ...
   else
     {
      //--------------------------------------------------------------
      // OPTION 2 : SUPERTREND
      // Calcul du ratchet sur barre fermee (index 1 et 2)
      // src = hl2 = (High + Low) / 2
      //--------------------------------------------------------------
      ...
      // Ratchet — les bandes ne peuvent pas reculer
      ...
      double new_up = (close2 > g_st_up)
                      ? MathMax(up_raw1, g_st_up)
                      : up_raw1;
      double new_dn = (close2 < g_st_dn)
                      ? MathMin(dn_raw1, g_st_dn)
                      : dn_raw1;

      // Mise a jour du trend
      if(g_st_trend == -1 && close1 > g_st_dn)
         g_st_trend =  1;
      else if(g_st_trend == 1 && close1 < g_st_up)
         g_st_trend = -1;

      // Sauvegarder les bandes mises a jour
      g_st_up = new_up;
      g_st_dn = new_dn;
   ...
   string trendMode = InpUseSupertrend
                      ? StringFormat("ST(trend:%+d)", g_st_trend)
      ...
      if(trade.Buy(InpLotSize, _Symbol, ask, sl, tp, "EMA_ADX_BUY"))
        {
         ...
         Print(StringFormat(
               ...
               " | ADX:%.1f | RSI:%.1f | ATR:%.3f | Trend:%s | BE:%s | Trail:%s",
