...
//|                    XAUUSD M5 - Version 2.0                      |
//|                                                                  |
//|  LOGIQUE ICT/SMC :                                              |
//|  - Swing Structure  : BOS / CHoCH (HH HL LH LL)                |
//|  - Internal Structure : BOS / CHoCH internes                    |
//|  - Order Blocks     : Swing OB + mitigation auto                |
//|  - Fair Value Gaps  : FVG bullish/bearish + auto threshold      |
//|  - EQH / EQL        : Equal Highs / Equal Lows                  |
//|  - Premium/Discount : Zones equilibre (midline EQ)              |
//|  - Signal BUY/SELL  : Confluence de tous les elements           |
//|                                                                  |
//|  Signal BUY  : BOS bull + Struct interne bull + Discount        |
//|                + OB bull + FVG bull + bougie verte + session    |
//|  Signal SELL : BOS bear + Struct interne bear + Premium         |
//|                + OB bear + FVG bear + bougie rouge + session    |
//+------------------------------------------------------------------+
#property copyright   "SMC_Gold_Scalper v2.0"
...
input int    InpOB_Max       = 3;    // Nb max d OB actifs affiches
...
input double InpFVG_MinUSD   = 2.0;  // Taille min FVG en USD
...
// Machine a etats - Structure swing
int    g_swing_trend  = 0;   //  1=bull -1=bear
int    g_int_trend    = 0;
double g_swing_top    = 0;
double g_swing_btm    = DBL_MAX;
double g_int_top      = 0;
double g_int_btm      = DBL_MAX;
int    g_top_x        = -1;
int    g_btm_x        = -1;
int    g_itop_x       = -1;
int    g_ibtm_x       = -1;
bool   g_top_cross    = false;
bool   g_btm_cross    = false;
bool   g_itop_cross   = false;
bool   g_ibtm_cross   = false;
...
// FVG storage
#define MAX_FVG 50
double   g_fvg_top[MAX_FVG];
double   g_fvg_btm[MAX_FVG];
datetime g_fvg_t1[MAX_FVG];
datetime g_fvg_t2[MAX_FVG];
...
// OB storage
#define MAX_OB 20
double   g_ob_top[MAX_OB];
double   g_ob_btm[MAX_OB];
datetime g_ob_t1[MAX_OB];
datetime g_ob_t2[MAX_OB];
int      g_ob_dir[MAX_OB];
bool     g_ob_mit[MAX_OB];
int      g_ob_count = 0;

// Dashboard
int    g_cnt_bos_bull  = 0;
int    g_cnt_bos_bear  = 0;
int    g_cnt_choch_bull= 0;
int    g_cnt_choch_bear= 0;
int    g_cnt_fvg_bull  = 0;
int    g_cnt_fvg_bear  = 0;
string g_last_signal   = "Aucun";
color  g_last_sig_clr  = clrSilver;
...
double ATR_approx(const int i, const double &h[], const double &l[],
                  const double &c[], const int p)
  {
   if(i < p) return 1.0;
   double atr = 0;
   for(int k = 1; k <= p; k++)
...
void DrawText(const string name, const datetime t, const double p,
              ...
              const int anchor = ANCHOR_BOTTOM)
  {
   string n = g_pfx + name;
   if(ObjectFind(0, n) >= 0) ObjectDelete(0, n);
...
void DrawRect(const string name, const datetime t1, const double hi,
              const datetime t2, const double lo, const color border,
...
void AddOB_Bull(const int i, const double &open[], const double &close[],
                ...
                const datetime &time[], const int total)
  {
   for(int j = i-1; j >= MathMax(0, i - InpSwing*3); j--)
     {
      if(close[j] < open[j])  // bougie rouge -> OB bullish
        {
         ...
         g_ob_count++;
         if(InpAlertOB) DoAlert("Order Block BULL", g_ob_top[idx], g_ob_t1[idx]);
         break;
        }
     }
  }
...
void AddOB_Bear(const int i, const double &open[], const double &close[],
                ...
                const datetime &time[], const int total)
  {
   for(int j = i-1; j >= MathMax(0, i - InpSwing*3); j--)
     {
      if(close[j] > open[j])  // bougie verte -> OB bearish
        {
         int idx = g_ob_count % MAX_OB;
         ...
         g_ob_count++;
         if(InpAlertOB) DoAlert("Order Block BEAR", g_ob_btm[idx], g_ob_t1[idx]);
         break;
        }
     }
  }
      ...
      if(g_ob_dir[o] == 1 && low[i] < g_ob_btm[o])
        {
         ...
         if(InpOB_HlightMit)
           {
            string sfx = "B_" + IntegerToString((int)g_ob_t1[o]);
            DrawRect("OB_"+sfx, g_ob_t1[o], g_ob_top[o], g_ob_t2[o], g_ob_btm[o],
                     C'80,80,80', C'30,30,30');
           }
        }
      if(g_ob_dir[o] == -1 && high[i] > g_ob_top[o])
        {
         ...
         if(InpOB_HlightMit)
           {
            string sfx = "S_" + IntegerToString((int)g_ob_t1[o]);
            DrawRect("OB_"+sfx, g_ob_t1[o], g_ob_top[o], g_ob_t2[o], g_ob_btm[o],
                     C'80,80,80', C'30,30,30');
           }
        }
   ...
   for(int i = start; i < rates_total; i++)
     {
      ...
      //================================================================
      //  1. SWING STRUCTURE : BOS / CHoCH
      //================================================================
      if(i < rates_total - InpSwing)
        {
         // Swing High
         if(IsSwingHigh(i, high, rates_total, InpSwing))
           {
            if(InpShowSwing)
               DrawText("SH_"+IntegerToString(i), time[i], high[i]+_Point*50,
                        (high[i] > g_swing_top) ? "HH" : "LH",
                        (high[i] > g_swing_top) ? clrLimeGreen : clrOrangeRed,
                        ANCHOR_BOTTOM);
            g_top_x     = i;
            g_top_cross = true;
         ...
         if(IsSwingLow(i, low, rates_total, InpSwing))
           {
            if(InpShowSwing)
               DrawText("SL_"+IntegerToString(i), time[i], low[i]-_Point*50,
                        (low[i] < g_swing_btm) ? "LL" : "HL",
                        (low[i] < g_swing_btm) ? clrOrangeRed : clrLimeGreen,
                        ANCHOR_TOP);
            g_btm_x     = i;
            g_btm_cross = true;
         ...
         if(g_top_cross && g_top_x >= 0 && close[i] > g_swing_top && g_swing_top > 0)
           {
            ...
            if(InpShowSwing)
              {
               ...
               DrawText("SBOS_BL_"+IntegerToString(i),
                        time[i], g_swing_top+_Point*30,
                        choch ? "CHoCH" : "BOS", clr, ANCHOR_BOTTOM);
              }
            g_swing_trend = 1;
            g_top_cross   = false;
            g_swing_top   = 0; // reset pour prochain HH
            ...
            if(InpShowOB) AddOB_Bull(i, open, close, high, low, time, rates_total);
           }
         ...
         if(g_btm_cross && g_btm_x >= 0 && close[i] < g_swing_btm && g_swing_btm < DBL_MAX)
           {
            ...
            if(InpShowSwing)
              {
               ...
               DrawText("SBOS_SL_"+IntegerToString(i),
                        time[i], g_swing_btm-_Point*30,
                        choch ? "CHoCH" : "BOS", clr, ANCHOR_TOP);
              }
            g_swing_trend = -1;
            g_btm_cross   = false;
            g_swing_btm   = DBL_MAX; // reset pour prochain LL
            ...
            if(InpShowOB) AddOB_Bear(i, open, close, high, low, time, rates_total);
           }
        }
            ...
            if(InpShowInternal)
              {
               DrawLine("IBOS_B_"+IntegerToString(i),
                        ...
                        clr, 1, STYLE_DOT);
               DrawText("IBOS_BL_"+IntegerToString(i),
                        time[i], g_int_top+_Point*20,
                        choch ? "i-CHoCH" : "i-BOS", clr, ANCHOR_BOTTOM);
              }
            g_int_trend  = 1;
            g_itop_cross = false;
            g_int_top    = 0;
            ...
            if(InpShowInternal)
              {
               DrawLine("IBOS_S_"+IntegerToString(i),
                        ...
                        clr, 1, STYLE_DOT);
               DrawText("IBOS_SL_"+IntegerToString(i),
                        time[i], g_int_btm-_Point*20,
                        choch ? "i-CHoCH" : "i-BOS", clr, ANCHOR_TOP);
              }
            g_int_trend  = -1;
            g_ibtm_cross = false;
            g_int_btm    = DBL_MAX;
      ...
      if(InpShowFVG && i >= 2 && i < rates_total - 1)
        {
         double atr    = ATR_approx(i, high, low, close, 14);
         double minFVG = InpFVG_Auto ? MathMax(InpFVG_MinUSD, atr * 0.1) : InpFVG_MinUSD;

         // FVG Bullish : high[i-2] < low[i]
         double gapB = low[i] - high[i-2];
         if(gapB >= minFVG)
           {
            ...
            DrawRect("FVG_B_"+IntegerToString(i),
                     ...
                     C'52,121,245', C'15,40,90');
            g_fvg_count++;
            g_cnt_fvg_bull++;
           }

         // FVG Bearish : low[i-2] > high[i]
         double gapS = low[i-2] - high[i];
         if(gapS >= minFVG)
           {
            int idx = g_fvg_count % MAX_FVG;
      ...
      for(int f = 0; f < MathMin(g_fvg_count, MAX_FVG); f++)
        {
         ...
         if(g_fvg_dir[f] ==  1 && low[i]  < g_fvg_btm[f]) g_fvg_mit[f] = true;
         if(g_fvg_dir[f] == -1 && high[i] > g_fvg_top[f]) g_fvg_mit[f] = true;
        }

      //================================================================
      //  5. EQUAL HIGHS / EQUAL LOWS
      //================================================================
      if(InpShowEQ && i >= InpSwing + 1)
        {
         if(IsSwingHigh(i, high, rates_total, InpSwing))
           {
                  ...
                  if(MathAbs(high[i]-high[j]) <= InpEQ_Thresh)
                    {
                     ...
                     DrawText("EQH_L_"+IntegerToString(i),
                              time[i], high[i]+_Point*40,
                              "EQH", clrOrangeRed, ANCHOR_BOTTOM);
                    }
                  break;
                 }
              }
                  ...
                  if(MathAbs(low[i]-low[j]) <= InpEQ_Thresh)
                    {
                     ...
                     DrawText("EQL_L_"+IntegerToString(i),
                              time[i], low[i]-_Point*40,
                              "EQL", clrDeepSkyBlue, ANCHOR_TOP);
                    }
                  break;
                 }
              }
      ...
      //================================================================
      //  6. PREMIUM / DISCOUNT - Midline EQ (ligne unique)
      //================================================================
      if(InpShowPD && i >= InpPD_Len && i % 25 == 0)
        {
         double hi  = high[ArrayMaximum(high, i-InpPD_Len+1, InpPD_Len)];
         double lo  = low[ArrayMinimum(low,   i-InpPD_Len+1, InpPD_Len)];
         double mid = (hi + lo) / 2.0;
         datetime ext = time[MathMin(i+50, rates_total-1)];

         // Ligne EQ centrale (pointillee grise)
         DrawLine("EQ_MID_"+IntegerToString(i),
                  time[i-25], mid, ext, mid,
                  C'120,120,120', 1, STYLE_DOT);
         DrawText("EQ_LBL_"+IntegerToString(i),
                  ext, mid, " EQ", C'120,120,120', ANCHOR_LEFT);
        }
      ...
      if(InpUseSession)
        {
         ...
         int h = dt.hour;
         bool inSess = (h >= InpLondon_Start && h < InpLondon_End) ||
...
void CreateDashboard()
  {
   ...
   string p = g_pfx + "D_";
   MkRect(p+"bg", x-6, y-6, 305, 325);
   MkLbl(p+"title", "  SMC GOLD SCALPER  v2.0",         x, y, clrGold, 11, "Arial Bold");
   y+=18; MkLbl(p+"sub", "  BOS + CHoCH + OB + FVG + EQ", x, y, C'100,100,160', 8);
   ...
   y+=13; MkLbl(p+"pd",  "Zone P/D  : --",                   x, y, clrSilver, 9);
   ...
   y+=15; MkLbl(p+"ver", "v2.0 | ICT/SMC | XAUUSD M5",      x, y, C'60,60,80', 8);
   ChartRedraw();
  }
...
void UpdateDashboard(const int rates_total, const datetime &time[],
                     const double &close[], const double &high[], const double &low[])
  {
   ...
   // Structure swing
   string strS = (g_swing_trend== 1) ? "BULL  [BOS haussier]" :
                 (g_swing_trend==-1) ? "BEAR  [BOS baissier]" : "NEUTRE";
   color  strC = (g_swing_trend== 1) ? clrLimeGreen :
                 (g_swing_trend==-1) ? clrOrangeRed : clrSilver;
   ObjectSetString(0,  p+"str", OBJPROP_TEXT,  "Structure : "+strS);
   ObjectSetInteger(0, p+"str", OBJPROP_COLOR, strC);

   // Internal
   string istS = (g_int_trend== 1) ? "BULL interne" :
                 (g_int_trend==-1) ? "BEAR interne" : "NEUTRE";
   color  istC = (g_int_trend== 1) ? clrLimeGreen :
                 (g_int_trend==-1) ? clrOrangeRed : clrSilver;
   ObjectSetString(0,  p+"ist", OBJPROP_TEXT,  "Int.Struct: "+istS);
   ObjectSetInteger(0, p+"ist", OBJPROP_COLOR, istC);

   // OB
   int obA = 0;
   for(int o = 0; o < MathMin(g_ob_count, MAX_OB); o++) if(!g_ob_mit[o]) obA++;
   ObjectSetString(0,  p+"ob", OBJPROP_TEXT,
      StringFormat("Order Blk : %d actif(s) / %d total", obA, g_ob_count));
   ObjectSetInteger(0, p+"ob", OBJPROP_COLOR, obA > 0 ? clrYellow : clrSilver);

   // FVG
   int fvgA = 0;
   for(int f = 0; f < MathMin(g_fvg_count, MAX_FVG); f++) if(!g_fvg_mit[f]) fvgA++;
   ObjectSetString(0,  p+"fvg", OBJPROP_TEXT,
      StringFormat("FVG       : %d actif(s) [B:%d S:%d]", fvgA, g_cnt_fvg_bull, g_cnt_fvg_bear));
   ObjectSetInteger(0, p+"fvg", OBJPROP_COLOR, fvgA > 0 ? clrYellow : clrSilver);
   ...
   if(last >= InpPD_Len)
     {
      ...
      else                  { pdS = "DISCOUNT [BUY zone]";  pdC = clrLimeGreen; }
     }
   ObjectSetString(0,  p+"pd", OBJPROP_TEXT,  "Zone P/D  : "+pdS);
   ...
   ObjectSetString(0,  p+"cnt", OBJPROP_TEXT,
      StringFormat("Stats     : BOS B:%d S:%d  CHoCH B:%d S:%d",
                   g_cnt_bos_bull, g_cnt_bos_bear,
                   g_cnt_choch_bull, g_cnt_choch_bear));
   ObjectSetInteger(0, p+"cnt", OBJPROP_COLOR, clrSilver);

   // Dernier signal
   ObjectSetString(0,  p+"sig", OBJPROP_TEXT,  "Signal    : "+g_last_signal);
   ObjectSetInteger(0, p+"sig", OBJPROP_COLOR, g_last_sig_clr);
   ...
   if(hr >= InpLondon_Start && hr < InpLondon_End)
     { bool isO = (hr*60+mn < InpLondon_Start*60+45);
       sS = isO ? "LONDRES [OUVERTURE *]" : "LONDRES [ACTIVE]";
       sC = isO ? clrGold : clrLimeGreen; }
   else if(hr >= InpNY_Start && hr < InpNY_End)
     { bool isO = (hr*60+mn < InpNY_Start*60+45);
       sS = isO ? "NEW YORK [OUVERTURE *]" : "NEW YORK [ACTIVE]";
       sC = isO ? clrGold : clrLimeGreen; }
